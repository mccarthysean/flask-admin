...
def test_add_user(test_app, monkeypatch):
    def mock_get_user_by_email(email):
        return None
    def mock_add_user(name, email, password):
        return True
    ...
    resp = client.post(
        "/users",
        data=json.dumps({"name": "michael", "email": "michael@email.io", "password": "test"}),
...
def test_update_user(test_app, monkeypatch):
    ...
    def mock_get_user_by_id(user_id):
        ...
        return d
    def mock_update_user(user, name, email):
        return True
    def mock_get_user_by_email(email):
        return None
    monkeypatch.setattr(app.api.users, "get_user_by_id", mock_get_user_by_id)
...
#     """assert that if a password is sent with a PUT request it does not 
#     update the password in the database"""

#     password_one = "greaterthaneight"
#     password_two = "somethingdifferent"

#     user = add_user("user-to-be-updated", "update-me@email.io", password_one)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            999,
            {"name": "me", "email": "me@email.io", "password": "greaterthaneight"},
            404,

...
from app.api.crud import get_user_by_email, get_user_by_id, add_user
...
full_user_model = auth_namespace.clone(
...
refresh_model = auth_namespace.model(
    "Refresh", {"refresh_token": fields.String(required=True)}
)

tokens_model = auth_namespace.clone(
    "Access and refresh_tokens", refresh_model, {"access_token": fields.String(required=True)}
)
...
class Register(Resource):
    ...
    @auth_namespace.expect(full_user_model, validate=True)
...
class Login(Resource):
    ...
    @auth_namespace.marshal_with(tokens_model)
...
class Refresh(Resource):
    ...
    @auth_namespace.marshal_with(tokens_model)
    @auth_namespace.expect(refresh_model, validate=True)
    @auth_namespace.response(200, "Success")

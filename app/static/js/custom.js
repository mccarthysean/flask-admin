...
$("td").click(function () {
  if (
    $(this).attr("class") == "list-buttons-column" ||
    $(this).attr("class") == "list-checkbox-column"
  ) {
    console.log("done");
    return;
  }
...
<li class="d-flex">
    ...
    <div class="form-group">
        <input type="color" class="form-control w-150 ml-5" id="color" onchange="fillcolor1(this)" list="presetColors">
...
<div class="form-group">
<input type="color" class="form-control w-150 ml-5" id="color" onchange="fillcolor2(this)" list="presetColors">
<datalist id="presetColors">
        <option>#0275d8</option>
        <option>#5cb85c</option>
        <option>#5bc0de</option>
...
function fillcolor1(val) {
  ...
  $.ajax({
    ...
    data: JSON.stringify({
      id_cell: $("#" + current_id).attr("data-id"),
      element: "font",
      color: val.value,
    }),
...
for (var i = 2; i < document.querySelectorAll("th").length; i++) {
  class_arr.push(
    document.querySelectorAll("th")[i].getAttribute("class").split(" ")[1]
  );
}
...
class_arr.forEach(function (class_elem) {
  var temp = 0;
  for (var i = 1; i < document.querySelectorAll("." + class_elem).length; i++) {
    if (
      document.querySelectorAll("." + class_elem)[i].querySelector("a>strong")
    ) {
      document
        .querySelectorAll("." + class_elem)
        [i].setAttribute(
          "data-id",
          class_elem +
            "_" +
            document
              .querySelectorAll("." + class_elem)
              [i].querySelector("a>strong").innerText +
            "_row_" +
            document.querySelectorAll(".elem")[temp].querySelector("a")
              .innerText
        );
    } else {
      if (document.querySelectorAll(".elem")[temp].querySelector("a")) {
        document
          ...
          [i].setAttribute(
            ...
            class_elem +
              "_" +
              document.querySelectorAll("." + class_elem)[i].innerText +
              "_row_" +
              document.querySelectorAll(".elem")[temp].querySelector("a")
                .innerText
          );
        ...
        document
          ...
          [i].setAttribute(
            ...
            class_elem +
              ...
              document.querySelectorAll("." + class_elem)[i].innerText +
              "_row_" +
              document.querySelectorAll(".elem")[temp].innerText
          );
      }
    }
    temp++;
